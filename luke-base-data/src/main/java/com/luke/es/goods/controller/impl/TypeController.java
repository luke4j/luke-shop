...
public class TypeController extends BController implements ITypeController {
    ...
    @Override
    public ActResult newKindGoodsCnf(HttpServletRequest request, HttpServletResponse response, ActResult actResult, DTOXtype dtoXtype, String jsonArrayLstDto, BindingResult bindingResult) throws Exception {
        List<VOXTypeGoodsCnf> lst = this.iTypeService.loadKindGoodsCnf(dtoXtype) ;
        VOXTypeGoodsCnf conf = new VOXTypeGoodsCnf();
        conf.setTypeKindId(dtoXtype.getId());
        lst.add(conf) ;
        return actResult.ok(conf);
    }
}
