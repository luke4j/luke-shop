...
public class Image {
    ...
    /**
     * 第二步：我们将原图和拉普拉斯锐化后的图进行相加操作，
     * 这样，我们就可以看到一样比较清晰的图了，下面是代码和运行效果
     * @param src
     * @return
     */
     ...
     * 第二步：Sobel提取边缘
     * 接下来，我们还是继续针对原图进行处理，
     * 我们想提取原图的边缘，Sobel是一阶算子，
     * 结果和图像卷积运算后，可以提取到图像的边缘信息，
     * 同样，下面给出代码和运行效果
     * @param src
     * @return
     */
    ...
    /**
     * 第三步：均值滤波
     * 这一步需要在Sobel变换后得到的图像基础上进行进一步的变换，
     * 均值滤波，就是将图像中每一点的像素值用其周边多个像素值的平均值重新赋值，
     * 有3*3，5*5的滤波器，我们即将使用的是5*5的滤波器，
     * 即是说，将某一个像素点重新赋值为以其为中心的25的像素值的平均值，
     * 下面我们看下代码和运行效果
     * @param src
     * @return
     */
    public BufferedImage meanValueProcess(BufferedImage src) {
        BufferedImage image = this.sobelProcess(src);// 已经索贝尔处理的图像
